...
function analyze() {
    ...
    xhr.onload = function(e) {
        if (this.readyState === 4) {
           ...
           console.log(response['names'][0], response['probs'][0]);
           console.log(response['names'][1], response['probs'][1]);
           console.log(response['names'][2], response['probs'][2]);

            // $('#pb').attr('aria-valuenow', response['probs'][0]).css('width', response['probs'][0]*100);
            // $('#pb1').attr('aria-valuenow', response['probs'][1]).css('width', response['probs'][1]*100);
            // $('#pb2').attr('aria-valuenow', response['probs'][2]).css('width', response['probs'][2]*100);
