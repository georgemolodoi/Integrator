...
# learn = loop.run_untill_complete(asyncio.gather(*tasks))[0]
# loop.close()
...
async def analyze(request):
    # Read image
    if request.method == 'POST' and request.FILS['myfile']:
        data = await request.FILES['myfile']
    ...
    return JSONResponse( {'names': cleanedNeams, 'probs': probs} )